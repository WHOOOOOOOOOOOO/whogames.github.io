...
void Game::update(Player& p)
{
	...
	if (p.m_x < -35)
	{
		p.m_x = 1025 - m_iAccelLeft;
	}
	if (p.m_x > 1035)
	{
		p.m_x = -25 + m_iAccelRight;
	}
	if (!m_bHover)
	{
		m_iJumpCtr++;
	...
	if (m_bLeftPressed)
	{
		if (m_iAccelLeft >= m_iAccelMax)
			m_iAccelLeft = m_iAccelMax;
		else
			m_iAccelLeft++;
		p.MoveX(-1 * 1 - (m_iAccelLeft / 5)/2);
		p.m_bRight = false;
	}
	else
	{
	...
	if (m_bRightPressed)
	{
		if (m_iAccelRight >= m_iAccelMax)
			m_iAccelRight = m_iAccelMax;
		else
			m_iAccelRight++;
		p.MoveX(1 * 1 + (m_iAccelRight / 5)/2);
		p.m_bRight = true;
